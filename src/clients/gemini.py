"""Minimal shared Gemini inference helper for lab projects.
...
from src.config import GEMINI_API_KEY, require_env
...
def _resolve_api_key(api_key: str | None = None) -> str:
    return require_env("GEMINI_API_KEY", api_key or GEMINI_API_KEY)
...
def _usage_count(usage: object, snake_name: str, camel_name: str) -> int:
    if usage is None:
        return 0
    value: Any = None
...
def extract_usage(response: object) -> GeminiUsage:
    """Normalize Gemini `usage_metadata` into local token buckets.

    Google reports `prompt_token_count`, `candidates_token_count`,
    `thoughts_token_count`, `cached_content_token_count`, and
    `total_token_count` on the response usage metadata. We store uncached prompt
    tokens separately from cached-content reads so cost math can price them
    differently.
    """
    ...
    prompt_tokens = _usage_count(usage, "prompt_token_count", "promptTokenCount")
    ...
    total_tokens = _usage_count(usage, "total_token_count", "totalTokenCount")
    if total_tokens <= 0:
        total_tokens = input_tokens + cache_read_tokens + output_tokens + thinking_tokens
    return GeminiUsage(
...
def _gemini_text_prices_per_1m(
    model_id: str,
    prompt_tokens: int,
) -> tuple[float, float, float] | None:
    """Return paid-tier standard text rates: input, output, cache-read per 1M."""
    # Source: Gemini Developer API pricing, paid-tier standard text rates.
    # Checked 2026-05-25.
    mid = model_id.lower()
    if "gemini-3.5-flash" in mid:
        return (1.50, 9.00, 0.15)
    if "gemini-3-flash-preview" in mid:
        return (0.50, 3.00, 0.05)
    if "gemini-3.1-pro-preview" in mid:
        if prompt_tokens > 200_000:
            return (4.00, 18.00, 0.40)
        return (2.00, 12.00, 0.20)
    if "gemini-3.1-flash-lite" in mid:
        return (0.25, 1.50, 0.025)
    return None


def compute_gemini_cost_usd(model_id: str, usage: GeminiUsage) -> float:
    prompt_tokens = usage.input_tokens + usage.cache_read_tokens
    prices = _gemini_text_prices_per_1m(model_id, prompt_tokens)
    if prices is None:
        return 0.0
    input_per_1m, output_per_1m, cache_read_per_1m = prices
    cost = (
        usage.input_tokens * input_per_1m
        + usage.cache_read_tokens * cache_read_per_1m
        + (usage.output_tokens + usage.thinking_tokens) * output_per_1m
    ) / 1_000_000.0
    return round(cost, 6)
...
def _generate_from_request(
    ...
    *,
    ...
    return GeminiResult(
        ...
        response=response,
        usage=usage,
        cost_usd=compute_gemini_cost_usd(request.model, usage),
    )
...
def embed_content(
    ...
    *,
    ...
    Retries transient Gemini failures (503 UNAVAILABLE, 429, 5xx, timeouts)
    with exponential backoff + jitter. This matches the behavior of
    `generate_text_with_retry` and `batch_embed_contents`, so single-shot
    callers (e.g. `embed_news_item` in the news pipeline) don't take down
    a 3-hour scheduler tick on a single Gemini blip.
    """
